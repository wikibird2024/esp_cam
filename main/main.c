...
#include "esp_psram.h"
#include "esp_flash.h"  // ✅ NEW: Required for esp_flash_get_size()
...
void start_webserver(void) {
  ...
  if (httpd_start(&server, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(server, &(httpd_uri_t){.uri = "/stream", .method = HTTP_GET, .handler = stream_handler});
...
void print_chip_info(void) {
  ...
  if (esp_psram_is_initialized()) {
    ESP_LOGI(TAG, "PSRAM: %dMB", psram / (1024 * 1024));
  } else {
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "Setup complete! Access the camera stream at http://<ESP_IP>/");
